...
function isAge55OrGreaterWithinCurrentYear(birthDate: Date): boolean {
  ...
  const endOfYearDateFiftyFiveYearsAgo = new Date(
    currentDate.getFullYear() - hsaCatchUpContributationCutoffAge,
    11,
    31,
  );
...
export default async function Home() {
  ...
  const hsaTableData = employeeRecords.records.map((record) => {
    ...
    const hsaMaxContribution = isAge55OrGreaterWithinCurrentYear(dateOfBirth)
